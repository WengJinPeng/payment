...
// Batch transfer file upload, parsing and batch creation
case 'batch_transfer_upload':
	if(!$conf['user_transfer'])exit('{"code":-1,"msg":"未开启代付功能"}');
	
	// Debug info
	file_put_contents('batch_debug.txt', date('Y-m-d H:i:s') . " - Starting batch transfer upload\n", FILE_APPEND);
	
	$app = isset($_POST['app']) ? $_POST['app'] : 'alipay';
	$paypwd = isset($_POST['paypwd']) ? trim($_POST['paypwd']) : '';
	
	// Validate password
	$pwdenc = getMd5Pwd($paypwd, $userrow['uid']);
	if(empty($paypwd) || $pwdenc!==$userrow['pwd'])exit('{"code":-1,"msg":"登录密码输入错误"}');
	
	// Check if file is uploaded
	if(!isset($_FILES['datafile']) || $_FILES['datafile']['error'] != 0)exit('{"code":-1,"msg":"请选择文件"}');
	
	// Get file info
	$filename = $_FILES['datafile']['tmp_name'];
	$filesize = $_FILES['datafile']['size'];
	$filetype = strtolower(pathinfo($_FILES['datafile']['name'], PATHINFO_EXTENSION));
	
	// Check file size
	if($filesize > 10*1024*1024)exit('{"code":-1,"msg":"文件大小不能超过10MB"}');
	...
	if($filetype == 'csv') {
		...
		if(($handle = fopen($filename, "r")) !== FALSE) {
			while(($row = fgetcsv($handle, 1000, ",")) !== FALSE) {
				if(count($row) < 3)continue; // At least 3 columns needed
				
				// Check for BOM and remove if necessary
				if($totalCount == 0 && substr($row[0], 0, 3) == pack('CCC', 0xef, 0xbb, 0xbf)) {
				...
				$username = trim($row[1]);
				$money = trim($row[2]);
				$desc = isset($row[3]) ? trim($row[3]) : '';
				
				// Validate data
				if(empty($account) || empty($username) || empty($money))continue;
				if(!is_numeric($money) || !preg_match('/^[0-9.]+$/', $money) || $money<=0)continue;
				
				// Specific validations based on payment type
				if($app == 'qqpay' && (!is_numeric($account) || strlen($account)<6 || strlen($account)>10))continue;
		...
		exit('{"code":-1,"msg":"不支持的文件格式，请上传CSV文件"}');
	}
	
	if(empty($data))exit('{"code":-1,"msg":"文件内容格式不正确或没有有效的代付数据"}');
	
	// Calculate total fee
	$totalFee = round((float)$totalAmount * (float)$conf['transfer_rate'] / 100, 2);
	$totalCost = round((float)$totalAmount + (float)$totalFee, 2);
	
	// Check available balance - we will actually deduct money only after transfers are complete
	...
	if($totalCost > $enable_money)exit(json_encode(['code'=>-1, 'msg'=>'余额不足，请充值后再试。需要：￥'.$totalCost.'，可用：￥'.$enable_money, 'available'=>$enable_money, 'required'=>$totalCost]));
	
	// Generate a unique batch number
	$batch_no = date("YmdHis").rand(111111,999999);
	...
	if(!$DB->insert('transfer_batch', $batch))exit('{"code":-1,"msg":"创建批量代付记录失败"}');
	...
	foreach($data as $item){
		// Removed problematic iconv/eval line
		try {
			$result = $DB->insert('transfer', [
			...
			if($result) {
				...
				// Capture error if insert fails
				$insert_error = $DB->error(); 
				// Log or handle specific item insertion failure if needed
				file_put_contents('batch_debug.txt', date('Y-m-d H:i:s') . " - Failed to insert item: " . json_encode($item) . " Error: " . $insert_error . "\n", FILE_APPEND);
			}
		} catch (Exception $e) {
			$insert_error = $e->getMessage();
			// Log or handle specific item insertion failure if needed
			file_put_contents('batch_debug.txt', date('Y-m-d H:i:s') . " - Exception inserting item: " . json_encode($item) . " Error: " . $insert_error . "\n", FILE_APPEND);
		}
	}
	
	// Check if any rows were successfully inserted
	if ($insert_success == 0) {
		// Optionally delete the batch record if no data was inserted
		$DB->delete('transfer_batch', ['id' => $batch_id]);
		exit('{"code":-1,"msg":"无法插入任何有效的代付数据，请检查文件格式和内容。 Error: '.$insert_error.'"}');
	}
	
	// We won't deduct money yet - we'll deduct for each successful transfer
	// Return total_count based on successfully inserted items
	exit(json_encode(['code'=>0, 'batch_id'=>$batch_id, 'total_count'=>$insert_success, 'total_amount'=>$totalAmount, 'total_fee'=>$totalFee]));
break;

// Finish batch processing
case 'batch_transfer_finish':
	if(!$conf['user_transfer'])exit('{"code":-1,"msg":"未开启代付功能"}');
	
	$batch_id = isset($_POST['batch_id']) ? intval($_POST['batch_id']) : 0;
	
	// Validate batch exists and belongs to user
	$batch = $DB->find('transfer_batch', '*', ['id'=>$batch_id, 'uid'=>$uid]);
	if(!$batch)exit('{"code":-1,"msg":"批量代付记录不存在"}');
	
	// Calculate processing time
	$endTime = microtime(true);
	$processTime = round($endTime - strtotime($batch['addtime']), 2);
	
	// Update batch status
	$DB->update('transfer_batch', ['status'=>1, 'process_time'=>$processTime], ['id'=>$batch_id]);
	
	// Get latest counts
	$success_count = $DB->getColumn("SELECT COUNT(*) FROM pre_transfer WHERE batch_id=:batch_id AND status=1", [':batch_id'=>$batch_id]);
	$fail_count = $DB->getColumn("SELECT COUNT(*) FROM pre_transfer WHERE batch_id=:batch_id AND status=2", [':batch_id'=>$batch_id]);
	$pending_count = $DB->getColumn("SELECT COUNT(*) FROM pre_transfer WHERE batch_id=:batch_id AND status=0", [':batch_id'=>$batch_id]);
	
	// Calculate total money for successful and pending transfers
	$success_amount = $DB->getColumn("SELECT SUM(money) FROM pre_transfer WHERE batch_id=:batch_id AND status=1", [':batch_id'=>$batch_id]) ?: 0;
	$pending_amount = $DB->getColumn("SELECT SUM(money) FROM pre_transfer WHERE batch_id=:batch_id AND status=0", [':batch_id'=>$batch_id]) ?: 0;
	
	// Calculate total cost to deduct now (only for successful and pending transfers)
	$total_process_amount = $success_amount + $pending_amount;
	$total_process_fee = round((float)$total_process_amount * (float)$conf['transfer_rate'] / 100, 2);
	$total_cost = round((float)$total_process_amount + (float)$total_process_fee, 2);
	
	// Deduct the total cost from user's balance
	if($total_cost > 0) {
		changeUserMoney($uid, $total_cost, false, '批量代付 #'.$batch['batch_no']);
	}
	
	// Drop the temporary table
	$table_name = $batch['ext'];
	if(!empty($table_name)) {
		$DB->exec("DROP TABLE IF EXISTS $table_name");
	}
	
	exit(json_encode(['code'=>0, 'success_count'=>$success_count, 'fail_count'=>$fail_count, 'pending_count'=>$pending_count, 'success_amount'=>$success_amount, 'total_cost'=>$total_cost]));
break;

// Get batch details with pagination
case 'batch_transfer_details':
	if(!$conf['user_transfer'])exit('{"code":-1,"msg":"未开启代付功能"}');
	...
	// Get transfer details with pagination
	$total = $DB->getColumn("SELECT COUNT(*) FROM pre_transfer WHERE batch_id=:batch_id", [':batch_id'=>$batch_id]);
	$list = $DB->getAll("SELECT * FROM pre_transfer WHERE batch_id=:batch_id ORDER BY biz_no DESC LIMIT $offset, $limit", [':batch_id'=>$batch_id]);
	
	exit(json_encode(['code'=>0, 'data'=>$list, 'total'=>$total]));
break;

// Process batch transfers in chunks
case 'batch_transfer_process':
	if(!$conf['user_transfer'])exit('{"code":-1,"msg":"未开启代付功能"}');
	...
	// Get appropriate channel ID based on payment type
	$app = $batch['type'];
	if($app=='alipay'){
	...
	// Get chunk of records to process
	$records = $DB->getAll("SELECT * FROM pre_transfer_data WHERE batch_id=:batch_id ORDER BY id ASC LIMIT $offset, $limit", [':batch_id'=>$batch_id]);
	// Debug information - Corrected table name reference
	$debug_info = [
		'batch_id' => $batch_id,
		'offset' => $offset,
		'limit' => $limit,
		'records_found' => $records ? count($records) : 0,
		'sql' => "SELECT * FROM pre_transfer_data WHERE batch_id=$batch_id ORDER BY id ASC LIMIT $offset, $limit",
		'batch_no' => $batch['batch_no'],
		'error' => $DB->error()
	];
	file_put_contents('batch_process_debug.txt', date('Y-m-d H:i:s') . " - Processing chunk: " . json_encode($debug_info) . "\n", FILE_APPEND);

	if(!$records || empty($records)) {
		// Check if there's any data left in the table for this batch
		$remaining_records = $DB->getColumn("SELECT COUNT(*) FROM pre_transfer_data WHERE batch_id=:batch_id", [':batch_id'=>$batch_id]);
		if($remaining_records > $offset) {
			// Should not happen if query is correct, but indicates an issue
			exit('{"code":-1, "msg":"查询处理数据时出错"}');
		} else {
			// No more records to process for this batch
			exit('{"code":0, "msg":"没有更多数据需要处理"}'); 
		}
	}
	...
	foreach($records as $item){
		...
		$biz_no = date("YmdHis").rand(11111,99999);
		
		// Calculate cost
		$costmoney = round((float)$money + ((float)$money * (float)$conf['transfer_rate']) / 100, 2);		
		...
		// Replace batch_submit with the standard submit function (assuming it exists and has similar params)
		$result = \lib\Transfer::submit($app, $channel, $biz_no, $account, $username, $money, $desc);
		
		// Check result structure carefully - adjust if needed based on actual \lib\Transfer::submit response
		if(isset($result['status']) && ($result['status']==1 || $result['status']==0)){ // status key exists and is 0 or 1
			$status = $result['status']; // 0=pending, 1=success, 2=failed (failure handled in else)
			$paytime = ($status == 1) ? date('Y-m-d H:i:s') : null; // Use current datetime for NOW() equivalent
			...
			$data = [
				...
				'status'=>$status, 
				'desc'=>$desc,
				'notify_url'=>$conf['localurl'].'pay/transfernotify/'.$channelid.'/'
			];
			...
			if($DB->insert('transfer', $data)!==false){
				...
				if($status == 0){
					$DB->exec("UPDATE pre_transfer_batch SET pending_count=pending_count+1 WHERE id=:id", [':id'=>$batch_id]);
				}elseif($status == 1){
					// Only deduct money for successful transfers here if needed, or handle in finish step
					$DB->exec("UPDATE pre_transfer_batch SET success_count=success_count+1 WHERE id=:id", [':id'=>$batch_id]);
				} 
				// Note: Status 2 (failure) is handled in the else block below
			} else {
				// Log error if insertion into pre_transfer fails
				file_put_contents('batch_process_debug.txt', date('Y-m-d H:i:s') . " - Failed to insert transfer record for biz_no $biz_no. DB Error: " . $DB->error() . "\n", FILE_APPEND);
				// Consider how to handle this - should it count as failure in the batch?
				// For now, let's increment fail count if DB insert fails after a successful/pending API call
				$DB->exec("UPDATE pre_transfer_batch SET fail_count=fail_count+1 WHERE id=:id", [':id'=>$batch_id]);
			}
		}else{ // Handle API call failure or unexpected response format
			...
			} elseif(is_string($result)) { // Sometimes error might be a plain string
				$error_msg = $result;
			}
			
			// Insert as failed transfer
			$data = [
				'biz_no'=>$biz_no, 
				'uid'=>$uid, 
				'batch_id'=>$batch_id,
				...
				'desc'=>$desc,
				'result'=>$error_msg, // Store the error message
				'notify_url'=>$conf['localurl'].'pay/transfernotify/'.$channelid.'/'
			];
			...
			if($DB->insert('transfer', $data)!==false){
				...
				// Log error if insertion into pre_transfer fails even for a failed API call
				file_put_contents('batch_process_debug.txt', date('Y-m-d H:i:s') . " - Failed to insert FAILED transfer record for biz_no $biz_no. DB Error: " . $DB->error() . "\n", FILE_APPEND);
			}
		}
	}
	
	exit(json_encode(['code'=>0, 'msg'=>'处理成功']));
break;
